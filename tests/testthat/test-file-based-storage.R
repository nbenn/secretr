...
test_that("file-based storage works", {
  ...
  expect_error(storeItem(cred2), paste0("the item to be stored is already ",
               "present."), fixed = TRUE)
  expect_equal(updateItem(cred2), NULL)
  expect_equal(fetchItem("test service"), cred2)
  expect_error(updateItem(cred3), paste0("did not find 1, but 0 matches"),
               fixed = TRUE)
  expect_equal(storeItem(cred3), NULL)
  expect_equal(length(readRDS(getOption("secretr.path"))), 2)
  expect_error(fetchItem("test service 3"), paste0("did not find 1, but 0 ",
               "matches"), fixed = TRUE)
  expect_error(removeItem("test service 3"), paste0("did not find 1, but 0 ",
               "matches"), fixed = TRUE)
